...
type Step = 'kategori' | 'form' | 'success';
...
function App() {
  ...
  const [showRules, setShowRules] = useState<KategoriLomba | null>(null);
                  ...
                  <div className="grid grid-cols-1 gap-2.5">
                    {grouped[tipe].map((k) => (
                      ...
                      >
                        ...
                        {k.peraturan && k.peraturan.length > 0 && (
                          <button
                            onClick={() => setShowRules(k)}
          ...
          <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[85vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 text-sm">Peraturan</h3>
                  <p className="text-xs text-gray-500">{showRules.nama}</p>
                </div>
              </div>
              <button
            ...
            <div className="overflow-y-auto p-4">
              <ol className="space-y-2.5">
                {showRules.peraturan?.map((rule, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="flex items-center justify-center w-6 h-6 rounded-full bg-red-100 text-red-700 text-xs font-semibold shrink-0">
                      {i + 1}
                    </span>
                    <span className="text-sm text-gray-700 leading-relaxed pt-0.5">{rule}</span>
                  </li>
                ))}
              </ol>
              <button
                onClick={() => {
                  const k = showRules;
                  setShowRules(null);
                  handleSelectKategori(k);
                }}
                className="w-full mt-5 inline-flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl px-4 py-3 text-sm shadow-sm transition-colors"
              >
                <Trophy className="w-4 h-4" /> Daftar Lomba Ini
              </button>
              {showRules.file && (
                <a
                  href={`/peraturan/${showRules.file}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full mt-2 inline-flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-xl px-4 py-3 text-sm border border-gray-200 transition-colors"
                >
                  <FileText className="w-4 h-4" /> Lihat PDF Peraturan
                </a>
              )}
            </div>
          </div>
